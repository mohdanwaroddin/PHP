<?php
	...
	if(isset($_POST['submit-vehicle'])) {
		...
		$q=mysqli_query($con,$sql="SELECT ID from vehicle_info where RegistrationNumber='$vehreno' and Status='' ");
		if($q){
			$patt="/^[A-Z]{2}[-][0-9]{4}$/";
			if(preg_match($patt,$vehreno)) {


		if(mysqli_num_rows($q)==0){
			 
		$query=mysqli_query($con, "INSERT into vehicle_info(ParkingNumber,VehicleCategory,VehicleCompanyname,RegistrationNumber,OwnerName,OwnerContactNumber,Status) values('$parkingnumber','$catename','$vehcomp','$vehreno','$ownername','$ownercontno','')") or die(mysqli_error($con));
		if ($query) {
	     ...
	     echo "<script>alert('Vehicle Already parked with this Reg No. ');</script>";
	       } } else {
	    
		echo "<script>alert('Invalid!, Enter Valid Reg No. ');</script>";
	         }
}}
  ?>
						...
						<div class="col-md-12">
							...
							<form method="POST">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" placeholder="MH-1869" id="vehreno" name="vehreno" required>
								</div>
